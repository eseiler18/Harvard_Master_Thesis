...
import torch
import numpy as np
from src.model import BuildNetwork_previous, BuildNetworkNew
import os
from pathlib import Path
current_path = Path.cwd().parent.parent
path = os.path.join(current_path, "model_history")
...
def save_model(trained_model, formatted_datetime_int, equation_name, name,
    ...
    # Save model history
    path_equation = os.path.join(path, equation_name)
    torch.save(trained_model.state_dict(), os.path.join(path_equation, f"{name}_{formatted_datetime_int}"))
    history = {}
    ...
    history["v"] = [v.cpu().numpy().tolist() for v in IC_list]
    ...
    with open(os.path.join(path_equation, f"history_{name}_{formatted_datetime_int}.json"), "w") as fp:
        print(os.path.join(path_equation, f"history_{name}_{formatted_datetime_int}.json"))
        json.dump(history, fp)


def load_run_history(equation_name, model_file, device, prev):
    path_equation = os.path.join(path, f"{equation_name}")
    with open(os.path.join(path_equation, "history_" + str(model_file) + ".json")) as f:
        history = json.load(f)
        ...
        IC_list = [torch.from_numpy(np.array(v)).to(device).double() for v in history["v"]]
        ...
        if prev:
            ...
            trained_model = BuildNetworkNew(1, hid_lay, num_equations, num_heads, device, activation="tanh", IC_list=IC_list).to(device, dtype=torch.double)
        trained_model.load_state_dict(torch.load(os.path.join(path_equation, str(model_file))))
        trained_model.eval()
    return trained_model, x_range, iterations, hid_lay, num_equations, num_heads, loss_hist, alpha_list, A_list, IC_list, force
